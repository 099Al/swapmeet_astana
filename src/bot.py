...
async def send_chat_rules(bot: Bot, message: Message, ctx: AppContext) -> None:
    ...
    try:
        await bot.send_message(
            ...
            text=(
                "Правила чата swapmeet_astana (Барахолка):\n\n"
                "1. Объявления размещаются только через бота @swapmeet_astana: /place или кнопка «Разместить объявление».\n"
                ...
                f"6. Лимиты: не больше {ctx.settings.user_hourly_ad_limit} объявлений в час,\n"
                f"не больше {ctx.settings.user_daily_ad_limit} объявлений в сутки,\n"
                f"не больше {ctx.settings.buy_daily_limit} объявлений на покупку в сутки,\n"
                f"до {ctx.settings.communication_daily_message_limit} сообщений в теме «Общение» в сутки."
            ),
        )
